...
export async function GET(
  ...
  try {
    const rows = await queryBridge<{ cofEmpCnpj: string }>(
      { bridgeUrl: l.sql_bridge_url, token },
      `SELECT cofEmpCnpj FROM config WHERE cofId = ${l.emp_id}`
    );

    const cnpj = rows[0]?.cofEmpCnpj ?? null;
    if (!cnpj) {
      return NextResponse.json({ error: "CNPJ não encontrado no ERP para este EmpId" }, { status: 404 });
    }

    return NextResponse.json({ success: true, cnpj });
  } catch (e) {
