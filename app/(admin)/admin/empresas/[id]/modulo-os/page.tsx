  ...
  const lojasData: LojaData[] = await Promise.all(
    lojas.map(async (loja) => {
      ...
      if (loja.sql_bridge_url && loja.sql_bridge_token) {
        ...
        const empId = loja.emp_id as number;
          ...
          <div className="border-t border-slate-100 px-6 pb-6 pt-5 space-y-8">
            ...
            <div className="space-y-3">
              ...
              {!loja.bridgeConfigurada ? (
                ...
                <div className="rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
                  <p className="text-xs text-slate-400">
                    Bridge configurada, mas nenhum inventário encontrado (bridge pode estar offline).
                  </p>
                </div>
              ) : (
                <form action={salvarInventario} className="flex items-end gap-3">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <select
                      ...
                      defaultValue={
                        loja.inventario_id_base != null ? String(loja.inventario_id_base) : "0"
                      }
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                ...
                entra imediatamente no cálculo de estoque fiscal — mesmo que a O.S ainda não
                tenha sido concluída. Marque apenas os tipos que representam consumo confirmado.
              </p>
              ...
              {!loja.bridgeConfigurada ? (
                ...
                <div className="rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
                  <p className="text-xs text-slate-400">
                    Nenhum tipo de atendimento encontrado (bridge pode estar offline).
                  </p>
                </div>
              ) : (
                <form action={salvarTiposFiscais} className="space-y-4">
                  ...
                  <div className="divide-y divide-slate-100 rounded-lg border border-slate-200">
                    {loja.tipos.map((tipo) => {
                      const corHex = tipo.tatCorDestaqueTexto
                        ? `#${tipo.tatCorDestaqueTexto}`
                        : "#64748b";
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          {/* Indicador de cor do ERP */}
                          <span
                            className="h-3 w-3 shrink-0 rounded-full border"
                            style={{
                              backgroundColor: corHex + "33",
                              borderColor: corHex,
                            }}
                          />
