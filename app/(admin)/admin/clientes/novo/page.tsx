...
// Formulário multi-seção para cadastrar um novo cliente com suporte a múltiplas lojas
...
interface FormState {
  ...
  // Seção 3 — Features premium ativas (core são sempre incluídas)
...
export default function NovoClientePage() {
  ...
  function adicionarLoja() {
    setLojas((prev) => [
      ...prev,
      // Copiar URL e terminal da primeira loja por conveniência (mesmo servidor)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Validar cada loja individualmente
    for (let i = 0; i < lojas.length; i++) { const loja = lojas[i];
    ...
    try {
      ...
      if (!res.ok) {
        setErroGeral(data.error ?? "Erro ao cadastrar cliente.");
        return;
      }
  ...
  if (sucesso) {
    return (
      <div className="flex flex-col items-center justify-center min-h-96 gap-4">
        ...
        <p className="text-xl font-semibold text-slate-800">Cliente cadastrado com sucesso!</p>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
          <div>
            <h2 className="font-semibold text-slate-800">Lojas / Empresas</h2>
          ...
          <p className="text-xs text-slate-400">
            Para clientes com múltiplas filiais: cada filial tem seu EmpId próprio no MaxData.
            A URL do túnel e o terminal geralmente são os mesmos para filiais no mesmo servidor.
          </p>
        </section>
        ...
        <section className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
                    usuarioPapel: e.target.value as "admin" | "viewer",
                  }))
                }
                className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-300"
              >
          ...
          <p className="text-xs text-slate-400">
            O usuário receberá as credenciais por e-mail. Solicite que troque a senha no primeiro
            acesso.
          </p>
        </section>
        ...
        >
          ...
          {loading ? "Cadastrando..." : `Cadastrar Cliente (${lojas.length} ${lojas.length === 1 ? "loja" : "lojas"})`}
        </button>
      </form>
    </div>
  );
